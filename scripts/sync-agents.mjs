 ...
 *
 * Usage: node scripts/sync-agents.mjs   (run from repo root; commit the output)
...
function syncAgent(filename) {
  ...
  const banner = `<!-- GENERATED from agents/${filename} by scripts/sync-agents.mjs — edit the source, then re-run. -->\n\n`;
...
// Remove generated files whose source no longer exists (renamed/deleted agents).
for (const stale of readdirSync(destDir).filter((f) => f.endsWith('.md') && !files.includes(f))) {
  unlinkSync(join(destDir, stale));
  console.log(`Removed stale cursor/agents/${stale}`);
}
