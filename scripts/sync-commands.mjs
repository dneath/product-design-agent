 ...
 *
 * Usage: node scripts/sync-commands.mjs   (run from the repo root; commit the output)
...
const banner = (name) =>
  `<!-- GENERATED from commands/${name}.md by scripts/sync-commands.mjs — edit the source, then re-run. -->`;
...
function toOpencode({ name, fm, body }) {
  let b = body
    ...
    .replaceAll(`${PLUGIN_ROOT}agent/product-design-partner.md`, 'agents/product-design-partner.md')
    .replaceAll(`${PLUGIN_ROOT}agent/modules/`, 'product-design-partner/modules/')
...
for (const { dir } of targets) {
  ...
  // Remove generated files whose source no longer exists (renamed/deleted commands).
  for (const stale of readdirSync(dir).filter((f) => f.endsWith('.md') && !files.includes(f))) {
    unlinkSync(join(dir, stale));
    console.log(`Removed stale ${dir.replace(repoRoot + '/', '')}/${stale}`);
  }
}
for (const file of files) {
  const cmd = parse(file);
  if (!cmd.fm.description) throw new Error(`${file}: missing description`);
  if (cmd.body.includes(PLUGIN_ROOT) && !cmd.body.includes(READ_LINE_CLAUDE)) {
    throw new Error(
      `${file}: uses \${CLAUDE_PLUGIN_ROOT} but is missing the exact "Read for method (…)" line — platform variants would keep Claude-only paths. Copy the sentence verbatim from another command.`
    );
  }
