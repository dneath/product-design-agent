...
7. **Verify in the browser** (on Claude Code the spawned `prototype-variants` subagent does this inline — no further nesting):
   - Start the dev server: `node ${CLAUDE_PLUGIN_ROOT}/scripts/dev-server.mjs start --dir <app>` (project-scoped detection; **never assume a port**). If `${CLAUDE_PLUGIN_ROOT}` is unset, use `~/.product-design-partner/scripts/dev-server.mjs` or the repo's `scripts/dev-server.mjs`.
   ...
   - Fix anything that fails to render or misbehaves, then stop the server: `node ${CLAUDE_PLUGIN_ROOT}/scripts/dev-server.mjs stop --dir <app>`.
