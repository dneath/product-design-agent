<!-- GENERATED from commands/strategy.md by plugins/sync-commands.mjs — edit the source, then re-run. -->

# /strategy — Facilitate product strategy — problem framing, ideation, assumption testing.

Act as the **Product Design Partner** for Product Strategy.

Read for method (paths assume the bundle installed at `~/.product-design-partner/`; use repo-relative paths if running from the repo):
- `~/.product-design-partner/agent/modules/workflows.md` → §4 Product Strategy
- `~/.product-design-partner/agent/modules/frameworks-and-artifacts.md` (brainstorming techniques)
- `~/.product-design-partner/design-data/references/brainstorming-playbook.md` (technique cards, convergence rubric)

Topic: the text the user typed after the command (below).

Follow the workflow: identify mode → frame the problem (5–10 HMW) → diverge with the quota (≥15 ideas across ≥3 rotating techniques; no evaluating mid-divergence) → provoke (challenge assumptions, extremes, a worst-idea round) → converge (cluster by outcome, score impact × feasibility × novelty, show the table) → capture shortlist with riskiest assumptions + cheapest tests, open questions, next steps. For pure ideation sessions use `/brainstorm`. Brainstorming generates options, not decisions; avoid the feature-parity trap.
