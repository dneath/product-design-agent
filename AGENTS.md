...
(all 5 boxes, shown, before any pixels or code), the routing table to the 9 modules, and the
cross-model rules. Load modules from `agent/modules/` and references from `design-data/references/`
as the routing table directs.
...
2. **Research before designing** unfamiliar things: shipping-product patterns + published UX
   evidence, then synthesize. Never design from memory alone. (`design-data/references/research-methods.md`)
...
the operating manual selects the module. Ten commands: `/design` `/prototype` `/brainstorm`
`/critique` `/design-system` `/handoff` `/deck` `/research` `/flows` `/figma-export`.
